...
- **Docker + LocalFs** _(по умолчанию)_ — Miharbor в контейнере, bind-mount директории с конфигом mihomo. Самый короткий путь до рабочей установки.
- **SSH** — Miharbor на рабочей станции / jump-хосте, деплой на удалённый mihomo через SSH (SFTP write + `mihomo -t` по exec, удалённый `flock` для локов). Подробности, env-переменные, auth-режимы (agent / key / passphrase-encrypted key) и troubleshooting — в [`docs/SSH_SETUP.md`](./docs/SSH_SETUP.md).
...
| `MIHARBOR_TRANSPORT`                | `local`                            | `local` (bind-mount) или `ssh` (удалённо по SSH — см. `docs/SSH_SETUP.md`). |

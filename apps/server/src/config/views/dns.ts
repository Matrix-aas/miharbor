// DNS view — projects a mihomo `dns:` section into a typed `DnsConfig`.
// Known fields are coerced to their declared types; unknown fields are
// preserved verbatim on `extras` so the UI can round-trip a doc that
// contains mihomo knobs Miharbor doesn't yet model.
...
function asNameserverPolicy(v: unknown): Record<string, string | string[]> | undefined {
  ...
  for (const [k, raw] of Object.entries(rec)) {
    if (typeof raw === 'string') {
      ...
      out[k] = raw.map((x) => String(x))
    }
    // ignore other shapes — linter will surface them separately
  }
  return out
}

/** Read the `dns:` section from `doc` and return a `DnsConfig`. Absent keys
 *  stay undefined; unknown keys land in `extras` so round-tripping preserves
 *  them. Safe to call on a doc without a `dns:` section (returns {}). */
